...
| `PUT` | Substituir/atualizar algo por completo | `PUT /api/devices/:id/settings` — atualiza as configurações do dispositivo |
...
O esquema completo vive em `backend/prisma/schema.prisma`. São 6 tabelas e 4 enums
...
(`settings` e `pump` **não** ficam mais no `User` — desde que passaram a ser por
dispositivo, ver `Setting`/`Pump` abaixo, cada relação mora em `Device`.)
...
- `settings Setting?` e `pump Pump?`: cada dispositivo tem sua **própria** configuração
  de limites e sua **própria** bomba d'água — dois sensores do mesmo usuário podem
  monitorar ambientes bem diferentes (uma estufa e uma geladeira, por exemplo), então não
  faria sentido uma única faixa "universal" valer para todos os dispositivos da conta.
  Isso não foi assim desde o início: numa versão anterior, `Setting` pendurava direto em
  `User` (1 configuração por conta inteira) — a mudança para 1-para-1 com `Device` veio
  depois, junto da adição do sensor de umidade do solo e da bomba (seção 5.8 e a nova
  seção sobre o módulo `pumps`).
...
- `onDelete: Cascade`: se o usuário dono for excluído, o PostgreSQL apaga
  ...
  tabela.
- `lastSeenAt`: atualizado a cada leitura recebida de verdade do ESP32 (`measurements.service.js`,
  seção 5.7) — é o único sinal que o frontend usa para decidir se um dispositivo está
  online, com comunicação atrasada ou offline (`utils/deviceStatus.js`, seção 6.4). Não
  existe mais um campo separado para distinguir leitura "real" de "simulada": desde que
  o endpoint de simulação foi removido do sistema (seção 5.7), toda leitura que chega já
  é, por definição, de um ESP32 de verdade.

### `Measurement` — cada leitura de temperatura, umidade do ar e umidade do solo
...
- `soilMoisture` é **opcional** (o `?`), diferente de `temperature`/`humidity`: nem todo
  dispositivo tem o sensor de umidade do solo instalado (só quem também controla uma
  bomba de irrigação, normalmente). Uma leitura sem esse sensor simplesmente grava `null`
  ali — o resto do sistema (status de solo, motor de alertas, bomba) sabe ignorar esse
  campo quando ele vem vazio, em vez de travar ou tratar como "zero".
...
### `Setting` — preferências de limites por dispositivo
...
`deviceId String @unique` é o que faz essa relação ser **1 para 1** (não 1 para N): o
Prisma não deixa existir duas linhas de `Setting` com o mesmo `deviceId`. Os três blocos
de campos (temperatura, umidade do ar, umidade do solo) seguem exatamente o mesmo
padrão — ideal + tolerância obrigatórios, mín/máx opcionais que substituem um lado do
cálculo quando definidos, e um interruptor de notificação independente por variável. A
lógica de como esses campos viram uma faixa mín/máx está em `settings.service.js`,
explicada na seção 5.8.

### `Alert` — os eventos de anomalia

```prisma
enum AlertVariable { temperature humidity soil_moisture }
...
`variable` inclui `soil_moisture` desde que a umidade do solo virou uma terceira
grandeza monitorada — o motor de alertas (seção 5.9) trata as três variáveis do mesmo
jeito, só pulando `soil_moisture` quando a leitura não trouxer esse sensor.

### `Pump` — a bomba d'água ligada ao sensor de umidade do solo

```prisma
enum PumpMode { automatic notify_only manual }
...
Assim como `Setting`, é 1-para-1 com `Device` — cada dispositivo tem no máximo uma
bomba. Os campos mais interessantes:

- `mode`: define o que acontece quando o solo fica seco. **`automatic`** liga o relé
  sozinha; **`notify_only`** nunca liga sozinha, só deixa o motor de alertas notificar
  (`Setting.notifySoilMoisture`); **`manual`** só muda de estado por comando explícito do
  usuário, em qualquer um dos três modos (o botão "Ligar agora"/"Desligar agora" da tela
  funciona sempre, independente do modo escolhido).
- `moistureThreshold`/`belowThresholdMinutes`: o "valor estipulado" e o "período
  estipulado" da regra — a umidade do solo precisa ficar **abaixo** de
  `moistureThreshold` por **pelo menos** `belowThresholdMinutes` minutos seguidos antes
  da bomba ligar sozinha (só em modo `automatic`).
- `isOn`/`turnedOnAt`: o estado atual do relé e desde quando está ligado — é isso que o
  firmware do ESP32 lê a cada leitura enviada para decidir se aciona o relé físico (seção
  7.5).
- `belowThresholdSince`: campo interno, não editável pelo usuário — marca desde quando a
  leitura mais recente está continuamente abaixo do limite. É zerado assim que uma
  leitura volta a ficar dentro do limite, para o "período estipulado" ser sempre contado
  a partir da seca **atual**, nunca acumulado entre janelas separadas.
...
└── modules/
    ...
    ├── measurements/        → ingestão do ESP32 (única fonte de leituras do sistema)
    ├── settings/            → temperatura/umidade ideal e tolerância (do ar e do solo)
    ├── alerts/              → o motor de alertas
    ├── pumps/               → estado e modo da bomba d'água (seção 5.8b)
...
Este módulo tem **uma única rota de entrada** para leituras, e ela só aceita um ESP32
de verdade — não existe (nem nunca mais existirá, ver seção 10) um jeito de gravar uma
leitura fingida:

```js
// Usado exclusivamente pelo ESP32 real — autenticado por device_id + X-Device-Key
router.post('/', measurementsLimiter, requireDeviceAuth, validateBody(createMeasurementSchema),
  async (req, res, next) => {
    ...
    res.status(201).json({ id: measurement.id, measured_at: measurement.measuredAt,
      received_at: measurement.receivedAt, pump: { isOn: pump.isOn } });
  });
```

Isso é uma mudança deliberada em relação a uma versão anterior do projeto, que também
tinha um endpoint `POST /measurements/simulate` (autenticado como usuário normal, para
gerar leituras aleatórias sem precisar do ESP32 ligado). Esse endpoint — e o mecanismo de
simulação automática do Dashboard que o chamava (seção 6.4) — foi **removido** assim que
o hardware real (ESP32 + DHT11 + sensor de solo + bomba) passou a funcionar de ponta a
ponta com confiança: manter os dois caminhos vivos só aumentava a chance de alguém
confundir um dado de demonstração com um dado real. Hoje, todo dado que aparece no
sistema veio de um ESP32 físico de verdade.
...
Depois de gravar a medição, `create()` sempre busca as configurações **daquele
dispositivo** (não mais do usuário — seção 4), calcula os limites (seção 5.8) e chama o
motor de alertas (seção 5.9) — toda leitura passa por essa mesma verificação. Se a
leitura trouxer `soilMoisture` (nem toda leitura traz, seção 4), também chama
`pumpsService.evaluateMeasurement()` (seção 5.8b), que decide se a bomba liga sozinha.

A resposta de sucesso (`201`, mostrada acima) já inclui o estado atual da bomba. É assim
que o firmware sabe se deve acionar o relé (seção 7.5) sem precisar de nenhuma
autenticação extra — ele já está autenticado por `X-Device-Key` nesta mesma requisição, e
o campo `pump.isOn` reflete tanto o modo automático quanto um comando manual dado pelo
app, o que vier mais recente.
...
function computeThresholds(settings) {
  ...
  const soilMoisture = { /* mesmo cálculo, com os campos idealSoilMoisture/soilMoistureTolerance/soilMoistureMin/Max */ };
...
28°C"). Umidade do ar e umidade do solo, quando calculadas automaticamente, são sempre
limitadas entre 0 e 100 (`Math.max`/`Math.min`) porque uma porcentagem fora dessa faixa
não existe fisicamente — temperatura não tem esse limite.
...
Desde que `Setting` passou a ser por **dispositivo** (seção 4), toda essa lógica é
idêntica — só muda a chave usada para buscar a configuração (`deviceId` em vez de
`userId`). Isso importa especialmente no histórico (`history.service.js`): ao listar
medições de vários dispositivos ao mesmo tempo, cada linha é avaliada contra os limites
do **seu próprio** dispositivo, nunca uma faixa "global" aplicada a todo mundo.

### 5.8b Módulo `pumps`: quando a bomba liga sozinha

`pumpsService.evaluateMeasurement(device, measurement)` roda depois do motor de alertas,
só quando a leitura trouxe `soilMoisture` (seção 5.7), e só ela decide o estado de
`Pump.isOn` além do endpoint de controle manual:

```js
async function evaluateMeasurement(device, measurement) {
  const pump = await getOrCreate(device.id);
  const isBelowThreshold = Number(measurement.soilMoisture) < Number(pump.moistureThreshold);

  if (!isBelowThreshold) {
    // volta ao normal: zera belowThresholdSince e, se estava ligada em modo
    // automatic, desliga sozinha também.
  } else if (pump.belowThresholdSince == null) {
    // primeira leitura seca da vez: só começa a contar o tempo, ainda não liga.
  } else if (pump.mode === 'automatic' && !pump.isOn) {
    // já vem contando: se o tempo decorrido >= belowThresholdMinutes, liga agora.
  }
}
```

Repare a assimetria proposital: **qualquer** modo conta havá quanto tempo o solo está
seco (`belowThresholdSince`) — isso alimenta a tela mesmo em modo `notify_only`/`manual`,
onde o usuário só quer *ver* essa informação. Mas só o modo `automatic` usa essa contagem
para de fato acionar o relé. É o mesmo motivo pelo qual a bomba também **desliga**
sozinha ao normalizar apenas nesse modo: nos outros dois, o estado do relé é inteiramente
responsabilidade do comando manual do usuário — o sistema nunca desliga uma bomba que o
próprio usuário ligou de propósito.

Igual ao motor de alertas, essa função roda para toda leitura com sensor de solo — o
`seed.js` do banco de demonstração até reaproveita a mesma função para gerar um histórico
de demonstração consistente (gravado direto no banco, não via API — uma janela de seca de
20 minutos que efetivamente liga a bomba automática de demonstração, e depois desliga
sozinha ao normalizar).
...
fazer com cada uma das três variáveis (temperatura, umidade do ar, umidade do solo)
separadamente:

```js
const VARIABLES = ['temperature', 'humidity', 'soilMoisture'];
// O enum do banco usa snake_case; o resto do código usa camelCase — este mapa só existe
...
async function evaluateMeasurement(measurement, device, thresholds, notifyFlags = { temperature: true, humidity: true, soilMoisture: true }) {
  for (const variable of VARIABLES) {
    if (measurement[variable] == null) continue; // sem sensor de solo nesta leitura: nada a avaliar
...
O parâmetro `notifyFlags` (vindo de
`Setting.notifyTemperature`/`notifyHumidity`/`notifySoilMoisture`, editável em
Configurações) só entra em ação **na hora de abrir um alerta novo** — repare na
...
de umidade do ar, faixa de umidade do solo, situação por variável) vira uma condição
independente adicionada à lista, o que evita esse tipo de conflito.

Como `Setting` é por dispositivo (seção 4), o filtro de "situação" (`normal`/`fora do
limite`) fica mais interessante quando a consulta abrange **vários** dispositivos ao
mesmo tempo (nenhum `deviceId` específico escolhido): em vez de uma única faixa aplicada
a todas as linhas, o backend monta um `OR` de sub-condições, uma por dispositivo, cada
uma já com o limite **daquele** dispositivo — assim, filtrar "temperatura fora do
limite" continua correto mesmo quando dois dispositivos têm faixas bem diferentes (uma
estufa e uma geladeira, por exemplo).
...
- Três `setInterval` independentes, dentro de `useEffect`, mantêm a tela atualizada sem
  o usuário precisar recarregar a página: a última leitura de cada dispositivo (a cada
  `POLL_INTERVAL_MS = 3_000`, 3 segundos), o gráfico do período selecionado (mesmo
  intervalo) e o estado da bomba (idem) — essa é a forma de "atualização em tempo real"
  escolhida no projeto: **polling**, não WebSocket. Repetir uma pergunta a cada poucos
  segundos é mais simples de implementar e depurar do que manter uma conexão aberta, e
  para o volume de dados de um TCC a diferença de eficiência é irrelevante. O intervalo
  foi reduzido de um valor inicial de 10s para 3s depois que o sistema passou a
  funcionar só com hardware real (ver abaixo) — sem a simulação preenchendo a tela
  continuamente, o atraso de até 10s entre uma leitura real do ESP32 chegar e aparecer
  na tela ficava perceptível demais.
- **Não existe mais simulação de leituras no Dashboard.** Uma versão anterior do projeto
  tinha um mecanismo que gerava leituras aleatórias a cada poucos segundos para
  dispositivos sem hardware físico enviando dados de verdade, útil para demonstrar o
  sistema sem depender do ESP32 estar ligado. Esse mecanismo (e o endpoint
  `POST /measurements/simulate` que ele chamava, seção 5.7) foi removido assim que o
  ESP32 real passou a funcionar de forma confiável — hoje, um dispositivo sem leituras
  recentes simplesmente aparece como offline (`utils/deviceStatus.js`), em vez de
  preencher a tela com dados fictícios.
- O card de "Última leitura" mostra três métricas quando o dispositivo tem sensor de
  solo habilitado (temperatura, umidade do ar, umidade do solo) ou duas quando não tem —
  o card de solo simplesmente não é desenhado se a leitura mais recente não trouxer
  `soilMoisture`. Ao lado dele, um painel de controle da bomba (só aparece se o
  dispositivo tiver `Pump` configurado) mostra o modo atual (automático/só notificar/
  manual), se está ligada, e um botão para ligar/desligar manualmente
  (`PUT /api/devices/:id/pump/toggle`) — o resultado desse toggle só é confirmado de
  fato quando o próximo `loadPump()` do polling roda, já que quem manda no relé de
  verdade é sempre o backend (seção 5.8b).
- O seletor de dispositivo (quando há mais de um) e o resumo compacto dos outros
  ...
  formas diferentes.
- O status do dispositivo (🟢 Online / 🟡 Sem comunicação recente / 🔴 Offline, em
  `utils/deviceStatus.js`) é calculado só a partir de `lastSeenAt`: até 30 segundos sem
  leitura nova é "Online", até 2 minutos é "Sem comunicação recente", acima disso é
  "Offline". Esses limiares também foram reduzidos (de 5 e 30 minutos) na mesma leva de
  ajustes citada acima — eram calibrados para um cenário em que a simulação preenchia a
  tela continuamente; com leituras reais chegando a cada poucos segundos
  (`READING_INTERVAL_MS` do firmware), esperar minutos para o status mudar deixava a
...
(mais os pares `src/Sensor.h`/`Sensor.cpp` e `src/Pump.h`/`Pump.cpp`, este último só
relevante em dispositivos com bomba d'água). Diferente do backend/frontend, não existe
...
### 7.2 A abstração do sensor (e da bomba)
...
struct SensorReading {
  ...
  bool valid;            // false se a leitura do DHT falhou ou veio fora da faixa plausível
...
Dentro de `Sensor.cpp`, a leitura do DHT11 é validada duas vezes: primeiro se é um número
...
de forma independente, no backend). O sensor de umidade do solo é opcional
(`SOIL_MOISTURE_ENABLED` em `config.h`) e independente do DHT: se desabilitado, o
firmware simplesmente não envia o campo `soilMoisture`, em vez de mandar zero ou travar a
leitura inteira por causa dele — a conversão do valor bruto do ADC (0-4095) para uma
porcentagem de 0 a 100 usa uma calibração seco/molhado (`SOIL_MOISTURE_DRY_RAW`/
`SOIL_MOISTURE_WET_RAW`) feita em ponto flutuante direto (não com o `map()` padrão do
Arduino, que trabalha em `long` e perderia precisão nesse cálculo).

`Pump.h`/`Pump.cpp` seguem a mesma ideia de abstração, só que para o relé da bomba
d'água: `pumpSetup()` configura o pino como saída e começa desligada; `pumpSetState(bool)`
aciona ou não o relé, respeitando `RELAY_ACTIVE_LOW` (a maioria dos módulos relé baratos
liga em nível **baixo**, o oposto do que se esperaria). O firmware nunca decide sozinho
quando ligar a bomba — só espelha no relé o que o backend mandou na resposta da última
leitura (seção 7.5b), então esse arquivo não tem nenhuma lógica de automação, só de
hardware.

Ambos os arquivos (`Sensor.cpp` e `Pump.cpp`) precisam incluir `config.h` explicitamente
(`#if __has_include("../config.h")`) para enxergar os `#define` que o usuário configurou
(`SOIL_MOISTURE_ENABLED`, `RELAY_PIN`, etc.) — um detalhe fácil de esquecer, porque o
Arduino compila cada arquivo de `src/` como uma unidade separada de `firmware.ino` (que é
quem inclui `config.h` "de fato"); sem esse include extra, os `#define` de `config.h`
simplesmente não apareceriam ali, e os valores padrão dos `#ifndef` seriam usados sempre,
ignorando silenciosamente o que foi configurado. Esse foi um bug real encontrado durante o
desenvolvimento — o sintoma era o sensor de solo parecer "não estar na programação" mesmo
com `SOIL_MOISTURE_ENABLED true` no `config.h`.
...
void sendMeasurement(const SensorReading &reading) {
  ...
  if (statusCode == 201) {
    applyPumpStateFromResponse(http.getString());
  }
  http.end();
}
```

Repare que `soilMoisture` só entra no JSON se `hasSoilMoisture` for verdadeiro — omitir a
chave (em vez de mandar `0` ou `null`) é o que faz o backend tratar aquele dispositivo
como "sem sensor de solo" nessa leitura, sem confundir com uma leitura real de solo seco
(seção 5.7).
...
### 7.6 Acionando a bomba a partir da resposta

```cpp
void applyPumpStateFromResponse(const String &response) {
  JsonDocument doc;
  DeserializationError err = deserializeJson(doc, response);
  if (err) return; // resposta não reconhecida: bomba mantém o último estado

  JsonVariant pumpIsOn = doc["pump"]["isOn"];
  if (pumpIsOn.isNull()) return;

  pumpSetState(pumpIsOn.as<bool>());
}
```

Quem decide se a bomba liga ou desliga é sempre o **backend** (modo automático por tempo
seco, só notificar, ou comando manual pelo app — seção 5.8b) — o firmware nunca toma essa
decisão sozinho, só lê o campo `pump.isOn` que já vem embutido na própria resposta de
sucesso da leitura que ele acabou de enviar (seção 5.7), sem precisar de nenhuma
requisição ou autenticação extra. Isso significa que um comando manual dado pelo app
(seção 6.4) só chega ao relé de fato na **próxima leitura enviada** pelo ESP32 — no pior
caso, um atraso de `READING_INTERVAL_MS` (5 segundos, por padrão). Se a resposta não vier
num formato reconhecível (erro de rede, servidor antigo sem esse campo), a bomba
simplesmente mantém o último estado conhecido — nunca liga ou desliga "no escuro".
...
- **Backend: Jest + Supertest**, comando `npm test` (dentro de `backend/`). Os arquivos
  ...
  | `pumps.test.js` | modos automático/só notificar/manual, contagem do tempo seco (`belowThresholdSince`), toggle manual em dispositivo novo (sem `Pump` criado ainda) |
...
- **Frontend: não existe uma suíte automatizada.** As telas foram verificadas
  ...
  login, geração de alertas enviando leituras reais, filtros de histórico, etc.), mas
  não há arquivos de teste automatizado de interface no repositório — nem Playwright, nem
  Cypress, nem Testing Library. Se o professor perguntar "e o frontend, como foi
  testado?", a resposta honesta é: manualmente, ponta a ponta (login, cadastro, geração
  de alertas enviando leituras reais para o ESP32 de testes, filtros de histórico,
  configuração da bomba etc.), e a garantia automatizada do projeto está concentrada na
  API (que é onde as regras de negócio realmente vivem — seção 2).
...
5. **`measurementsService.create()`** (seção 5.7): resolve o `measuredAt`, grava a
   medição no banco (`prisma.measurement.create`) e atualiza `lastSeenAt` do dispositivo.
...
9. **No navegador**, o Dashboard (aberto pelo usuário) está fazendo `GET
   /api/measurements/latest` a cada 3 segundos (seção 6.4). Na próxima rodada desse
...
Esse é o único caminho que existe hoje para uma leitura chegar ao sistema — não há mais
um caminho alternativo "simulado" com um passo 3 diferente (seção 6.4).
...
**"O sistema já teve um jeito de simular leituras sem o ESP32 ligado? Por que foi tirado?"**
Sim — uma versão anterior tinha um endpoint `POST /measurements/simulate` e um
mecanismo no Dashboard que gerava leituras aleatórias automaticamente, para permitir
demonstrar o fluxo completo (leitura → dashboard → histórico → alerta) sem depender do
hardware físico estar ligado e por perto. Ele foi removido assim que o ESP32 real (com
DHT11, sensor de solo e bomba) passou a funcionar de ponta a ponta com confiança: manter
os dois caminhos vivos aumentava a chance de confundir um dado de demonstração com um
dado real, e deixava de forçar o projeto a provar que o hardware funciona de verdade
(seção 5.7). Hoje, toda leitura que aparece no sistema veio de um ESP32 físico.
...
**"Por que o Dashboard atualiza sozinho a cada poucos segundos em vez de usar WebSocket?"**
Foi uma escolha consciente de simplicidade: *polling* (perguntar de novo periodicamente,
hoje a cada 3 segundos) é mais simples de implementar, testar e depurar do que manter uma
conexão persistente aberta, e para o volume de dados e a escala de um projeto de TCC a
diferença de eficiência entre as duas abordagens é irrelevante na prática (seção 6.4).

**"Como o sistema decide que um dispositivo está online, atrasado ou offline?"**
Só pelo tempo desde a última leitura recebida (`Device.lastSeenAt`): até 30 segundos sem
notícia é "Online", até 2 minutos é "Sem comunicação recente", mais que isso é "Offline"
(`utils/deviceStatus.js`, seção 6.4). Não há nenhum aviso vindo do próprio ESP32 quando
ele desliga — é só uma inferência por ausência de dado.

**"Como fica o `config.h` se eu tiver mais de um ESP32?"**
`config.h` é um arquivo por sketch — cada ESP32 recebe seu próprio upload com seu próprio
`WIFI_SSID`/`DEVICE_ID`/`DEVICE_TOKEN`. Na prática, isso significa manter uma cópia de
`config.h` por dispositivo fora da pasta do firmware (ex.: `config-estufa.h`,
`config-geladeira.h`) e copiar a certa para `esp32/firmware/config.h` antes de cada
upload — o arquivo em si nunca muda de nome nem de lugar, porque é esse nome fixo que o
`firmware.ino` inclui (`#if __has_include("config.h")`, seção 7).
