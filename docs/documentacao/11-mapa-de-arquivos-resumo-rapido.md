...
| `firmware/src/Sensor.cpp` | Implementação do DHT11 + sensor opcional de umidade do solo |
| `firmware/src/Pump.h` | Interface genérica do relé da bomba (`pumpSetup`, `pumpSetState`) |
| `firmware/src/Pump.cpp` | Implementação do relé (pino, nível ativo) |
...
| Arquivo | O que é |
|---|---|
| `schema.prisma` | As 6 tabelas do banco (User, Device, Measurement, Setting, Alert, Pump) |
...
| `devices/` | CRUD de dispositivos ESP32 + rotas de settings/pump por dispositivo |
| `measurements/` | Recebe leituras só do ESP32 real, calcula `measuredAt` |
| `settings/` | Limites ideais/tolerância por dispositivo (ar e solo), calcula a faixa aceitável |
| `alerts/` | O motor de alertas: abre/atualiza/fecha eventos de anomalia |
| `pumps/` | Estado e modo da bomba d'água (automático/só notificar/manual) |
...
| `pumps.test.js` | Modos da bomba, contagem do tempo seco, toggle manual |
...
| `Dashboard.jsx` | Cartões (temp./umidade/solo), gráficos, controle da bomba, polling 3s |
| `History.jsx` | Tabela paginada com filtros e exportação CSV |
| `Settings.jsx` | Limites ideais/tolerância (temp./umidade/solo) + modo da bomba + aparência |
...
| `users.js` | `/users/me*` |
| `devices.js` | `/devices*` |
| `measurements.js` | `/measurements/latest` |
| `settings.js` | `/devices/:id/settings` |
| `pumps.js` | `/devices/:id/pump*` |
| `alerts.js` | `/alerts*` |
...
| `deviceStatus.js` | Classifica dispositivo em online (30s)/atenção (2min)/offline |
