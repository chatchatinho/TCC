...
│   │   ├── middlewares/
│   │   └── modules/
│   │       ├── auth/ users/ devices/ measurements/ settings/ alerts/ pumps/ history/
│   └── tests/
├── frontend/
│   ├── package.json / package-lock.json
...
- **`schema.prisma`** — o arquivo mais importante do banco: define as 6 tabelas
...
- **`seed.js`** — script (`npm run db:seed`) que **apaga tudo** (`alert.deleteMany()`,
  ...
  leitura a cada 5 minutos, incluindo umidade do solo, com uma janela de ~20 minutos de
  temperatura fora do limite no meio) e um alerta já resolvido correspondente a essa
  janela, marcado como não lido — para já aparecer a notificação "você possui alertas
  desde seu último acesso" assim que alguém logar com esse usuário de teste. A mesma
  janela de solo seco também é avaliada pela `pumpsService.evaluateMeasurement()`
  (seção 5.8b do documento 09) para deixar a bomba de demonstração num estado
  consistente com o histórico gerado (chega a ligar e desligar sozinha, em modo
  automático).
- **`migrations/`** — o histórico, passo a passo, de como o esquema do banco evoluiu.
  ...
  só uma nova migration é criada em cima. As oito existentes, em ordem:
  ...
  | `20260821204801_add_last_real_measurement_at` | Adiciona `last_real_measurement_at` em `devices` (distinção leitura real vs. simulada — removido depois, ver a migration de remoção da simulação abaixo) |
  ...
  | `20260909234624_settings_per_device` | Move `settings` de 1-por-`user` para 1-por-`device` |
  | `20260910000514_soil_moisture_and_pump` | Adiciona `soil_moisture` em `measurements`, os 5 campos de solo em `settings`, o valor `soil_moisture` no enum `AlertVariable`, e as tabelas/enum novos `pumps`/`PumpMode` |
  | `20260910015406_remove_simulation_and_last_real_measurement` | Remove `devices.last_real_measurement_at` — sem o endpoint de simulação (seção 3.3, `measurements/`), não existe mais leitura "simulada" para distinguir de uma "real" |
...
**`devices/`** (CRUD de ESP32s — e também onde moram as rotas de `settings`/`pumps`,
já que ambos são configurações "de um dispositivo específico")
- `devices.routes.js` — `GET`/`POST /devices`, `PUT`/`DELETE /devices/:id`,
  `POST /devices/:id/rotate-secret`, `GET`/`PUT /devices/:id/settings` (chamando
  `settingsService`), `GET`/`PUT /devices/:id/pump` e `POST /devices/:id/pump/toggle`
  (chamando `pumpsService`) — os dois últimos módulos não têm arquivo de rotas próprio.
...
**`measurements/`** (ingestão de leituras — única fonte de dados do sistema)
- `measurements.routes.js` — só `POST /measurements` (autenticação de dispositivo, para
  o ESP32 real) e `GET /measurements/latest` (última leitura de cada dispositivo do
...
- `measurements.service.js` — `create()` (grava a medição — incluindo `soilMoisture`
  ...
  ausente/implausível vindo do ESP32), `latestByUser()`.
- `measurements.validation.js` — faixa fisicamente plausível para a leitura bruta
  (-40°C a 80°C, 0% a 100% de umidade do ar, 0% a 100% de umidade do solo quando
  enviada — mais ampla que a faixa nominal do DHT11 de propósito, só rejeitando o
  fisicamente impossível).

**`settings/`** (limites de temperatura/umidade/solo, por dispositivo — sem
...
- `settings.validation.js` — faixas plausíveis para cada campo (temperatura, umidade do
  ar, umidade do solo) e regras cruzadas (`refine`): a taxa mínima precisa ser menor que
  a máxima, em cada variável.

**`pumps/`** (bomba d'água — automática, só notificar, ou manual — também sem
`pumps.routes.js` próprio: as rotas moram em `devices.routes.js`, ver abaixo)
- `pumps.service.js` — `getOrCreate()` (cria a configuração padrão da bomba na primeira
...
- **`pumps.test.js`** — modos automático/só notificar/manual, contagem do tempo seco,
  toggle manual num dispositivo recém-criado (sem `Pump` ainda).
...
- **`README.md`** — documentação de uso do frontend isoladamente: configuração,
  ...
  polling em vez de WebSocket, fuso horário).
...
- **`Dashboard.jsx`** — a página mais complexa do frontend: cartões de métrica
  (temperatura, umidade do ar e, quando o dispositivo tem o sensor, umidade do solo),
  painel de controle da bomba d'água, gráficos de linha com seletor de período, seletor
...
- **`Settings.jsx`** — por dispositivo (um seletor no topo da tela quando há mais de
  um): abas "Valores" (ideal/tolerância obrigatórios por variável — temperatura, umidade
  do ar, umidade do solo —, taxa mín/máx opcional, prévia da faixa calculada,
  interruptor de notificação por variável), "Bomba d'água" (modo automático/só
  notificar/manual, limite de umidade, minutos de espera) e "Aparência" (tema, cor de
  destaque, tamanho de fonte, redução de movimento).
...
- **`devices.js`** — `listDevices`, `createDevice`, `updateDevice`, `deleteDevice`,
  `rotateDeviceSecret`.
- **`measurements.js`** — só `getLatest` (não existe mais `simulateMeasurement` — o
  endpoint de simulação foi removido, seção 3.3).
- **`settings.js`** — `getSettings(deviceId)`, `updateSettings(deviceId, data)` —
  ambos por dispositivo, batendo em `/devices/:id/settings`.
- **`pumps.js`** — `getPump(deviceId)`, `updatePump(deviceId, data)`,
  `togglePump(deviceId, isOn)` — batendo em `/devices/:id/pump*`.
...
- **`deviceStatus.js`** — `getDeviceStatus()`: classifica um dispositivo como
  online (🟢, comunicou nos últimos 30 segundos), "sem comunicação recente" (🟡, até 2
  minutos) ou offline (🔴, mais que isso, ou nunca comunicou). Limiares reduzidos de 5
  e 30 minutos depois que o sistema passou a depender só de leituras reais do ESP32
  (que chegam a cada poucos segundos) — os valores antigos deixavam o status "preso"
  em Online por tempo demais depois que o dispositivo já tinha ficado sem comunicação.
