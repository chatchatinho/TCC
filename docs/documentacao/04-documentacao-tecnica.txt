...
2. **Loop periódico** (a cada `READING_INTERVAL_MS`, configurável): lê o sensor DHT11 e,
   opcionalmente, um sensor de umidade do solo, através de uma camada de abstração
   (`src/Sensor.h`/`Sensor.cpp`) que isola as bibliotecas específicas do resto do
   firmware — trocar de sensor no futuro exige mexer só nesse arquivo.
3. A leitura é **validada localmente** (descarta NaN e valores fora de uma faixa
   plausível) antes de qualquer envio.
4. Se válida, monta um JSON (`device_id`, `temperature`, `humidity`, `soilMoisture`
   quando o sensor de solo está habilitado, `timestamp`) e envia por `HTTPClient` com o
   header `X-Device-Key`.
5. Trata a resposta (sucesso, erro do servidor, falha de rede) sem travar — sempre volta
   ao loop e tenta de novo no próximo ciclo. Numa resposta de sucesso, lê o campo
   `pump.isOn` e aciona o relé da bomba d'água (`src/Pump.h`/`Pump.cpp`) de acordo — quem
   decide ligar/desligar é sempre o backend, o firmware só espelha esse estado.
6. Se o Wi-Fi cair, tenta reconectar periodicamente (não bloqueia o restante do loop
   indefinidamente).

Não existe mais nenhum caminho para gravar uma leitura "de mentira": um endpoint de
simulação e um gerador automático de leituras no Dashboard existiram numa versão
anterior do projeto e foram removidos assim que o hardware real passou a funcionar de
ponta a ponta — hoje, toda leitura do sistema vem de um ESP32 físico.
...
erDiagram
    ...
    DEVICES ||--|| SETTINGS : configura
    DEVICES ||--o| PUMPS : controla
    ...
    MEASUREMENTS {
        ...
        decimal temperature
        decimal humidity
        decimal soil_moisture "opcional"
        timestamptz measured_at
        timestamptz received_at
    }
    SETTINGS {
        uuid id PK
        uuid device_id FK "1 configuração por DISPOSITIVO"
        decimal ideal_temperature
        decimal temperature_tolerance
        decimal ideal_humidity
        decimal humidity_tolerance
        decimal ideal_soil_moisture "opcional"
        decimal soil_moisture_tolerance "opcional"
    }
    PUMPS {
        uuid id PK
        uuid device_id FK
        text mode "automatic | notify_only | manual"
        boolean is_on
        decimal moisture_threshold
        int below_threshold_minutes
        timestamptz below_threshold_since "nullable"
    }
    ALERTS {
        uuid id PK
        uuid user_id FK
        ...
        text variable "temperature | humidity | soil_moisture"
        text direction
        decimal peak_value
        decimal limit_min
        decimal limit_max
...
Duas mudanças em relação a uma versão anterior do modelo: `Settings` deixou de ser
1-para-1 com `User` e passou a ser 1-para-1 com `Device` (dois dispositivos do mesmo
usuário podem monitorar ambientes com faixas bem diferentes — uma estufa e uma
geladeira, por exemplo); e `Device.last_real_measurement_at` foi removido junto da
remoção do endpoint de simulação (seção 9) — sem simulação, não existe mais distinção
entre leitura "real" e "simulada" a rastrear.
...
## 9b. Bomba d'água (irrigação automática)

Dispositivos com sensor de umidade do solo podem controlar uma bomba (relé) em três
modos independentes, configuráveis por dispositivo em Configurações → Bomba d'água:

- **Automático**: liga sozinha quando a umidade do solo fica abaixo de
  `moistureThreshold` por, no mínimo, `belowThresholdMinutes` minutos seguidos; desliga
  sozinha assim que a leitura volta a ficar acima do limite.
- **Só notificar**: continua contando o tempo seco (para a tela mostrar essa informação),
  mas nunca liga/desliga a bomba sozinha — só quem decide é o usuário.
- **Manual**: o relé só muda de estado por comando explícito do usuário
  (`PUT /devices/:id/pump/toggle`); o sistema nunca liga ou desliga uma bomba que o
  usuário controla manualmente.

O campo `Pump.belowThresholdSince` modela "desde quando a leitura mais recente está
continuamente abaixo do limite" como um evento (mesma ideia do motor de alertas, seção
9) — é zerado assim que uma leitura volta ao normal, para o período estipulado ser
sempre contado a partir da seca **atual**. O firmware do ESP32 nunca decide sozinho:
ele só lê o campo `pump.isOn` devolvido na resposta de cada leitura enviada e aciona o
relé físico de acordo (`esp32/README.md`).

## 10. Gráficos

Dashboard e histórico usam Chart.js (via `react-chartjs-2`) com gráficos de linha para
temperatura, umidade do ar e umidade do solo. O eixo X usa rótulos de horário já
formatados no fuso de São Paulo (não uma escala de tempo contínua), porque o volume de
pontos exibido é sempre limitado pelo filtro de período aplicado na consulta à API
(seção 11) — evita depender de uma biblioteca adicional só para eixo temporal.
Atualização a cada 3s por polling (reduzido de um valor inicial de 10s depois que o
sistema passou a depender só de hardware real — ver decisão na seção 24 do escopo,
registrada em
[`01-arquitetura-e-decisoes.md`](01-arquitetura-e-decisoes.md#3-justificativa-das-escolhas)).
O status de conexão de cada dispositivo (online/sem comunicação recente/offline) também
usa limiares curtos pelo mesmo motivo: até 30 segundos sem leitura nova é "Online", até
2 minutos é "Sem comunicação recente", mais que isso é "Offline"
(`frontend/src/utils/deviceStatus.js`).
...
103 testes automatizados (Jest + Supertest) contra um banco de teste real, cobrindo
cadastro, login, isolamento entre usuários, ingestão de medições, o motor de alertas
(incluindo a lógica anti-spam), a bomba d'água (modos automático/só notificar/manual) e
histórico — ver
