...
Use um intervalo curto (5 segundos, o padrão) e não algo como 30 segundos: o sistema
considera um dispositivo "Online" só enquanto ele mandar uma leitura a cada 30 segundos
ou menos — um intervalo maior que isso faz o status ficar piscando entre 🟢 Online e 🟡
Sem comunicação recente a cada ciclo.
...
Volte para `http://localhost:5173`, vá em **Dashboard** — a leitura do sensor real deve
aparecer no card de Temperatura/Umidade em até 5 segundos (o intervalo configurado no
`config.h`, mais até 3 segundos do polling da própria tela), e o status do dispositivo
deve virar **🟢 Online**.
...
## 13. Opcional: sensor de umidade do solo e bomba d'água

Este guia cobre só o DHT11 (temperatura/umidade do ar), que é o mínimo para o sistema
funcionar. Se o seu dispositivo também tiver um sensor de umidade do solo e/ou um relé
de bomba d'água, a fiação, os campos extras de `config.h`
(`SOIL_MOISTURE_ENABLED`/`SOIL_MOISTURE_PIN`/`RELAY_PIN`/`RELAY_ACTIVE_LOW`) e a
calibração seco/molhado do sensor de solo estão documentados em
[`esp32/README.md`](../../esp32/README.md) — o firmware já é o mesmo, só a configuração
muda.
