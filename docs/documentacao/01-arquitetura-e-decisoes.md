...
| Testes frontend | Vitest + React Testing Library (básico) — planejado nesta etapa, mas não chegou a ser implementado; a verificação do frontend acabou sendo só manual (ver seção 8 do documento 09) |
...
4. **Múltiplos dispositivos por usuário** → o **banco de dados é modelado para suportar vários** (FK simples em `devices.user_id`, sem unicidade). Na primeira versão, o frontend assumia um único dispositivo ativo por usuário (sem seletor no dashboard). Isso foi revisto depois: a tela de "Dispositivos" permite cadastrar quantos o usuário quiser, e o Dashboard mostra um seletor de "Dispositivo" (dropdown) sempre que houver mais de um — escondido quando há só um, para não adicionar uma tela extra a quem não precisa dela. Trocar de dispositivo no seletor troca os cards, o gráfico e a simulação automática para o dispositivo escolhido.
